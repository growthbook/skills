...
- `learnings` — search what the team has already concluded before designing. If a Learning already settles the question, or contradicts the hypothesis, surface it rather than designing a test that re-litigates it. Most orgs have no Learnings yet; an empty result means nothing has been recorded, not that there's no relevant history — fall through to `experiment-brainstorm` for the experiment record itself.
