...
All API calls go through the bundled helper: `${CLAUDE_PLUGIN_ROOT}/scripts/gb-call`. It needs `GB_API_KEY` (and `GB_EMAIL` for the approval-required branch) set in env or written to `~/.config/growthbook/.env` by `/growthbook:setup`.
