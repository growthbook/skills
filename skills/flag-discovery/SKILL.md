...
2. If the user wants details on a subset, fetch them paginated:
   ```bash
   gb-call GET '/api/v1/features?limit=100'
   gb-call GET '/api/v1/features?limit=100&offset=100'    # next page
   ```
...
```bash
gb-call GET /api/v1/features/<flag-id>
```

Surface: default value, environments enabled, current rules, owner, tags, last modified.
...
2. Call the staleness audit with the IDs as a comma-separated query param:
   ```bash
   gb-call GET '/api/v1/stale-features?ids=flag-a,flag-b,flag-c'
   ```

3. Present the report grouped by recommendation: launched (safe to remove), unused, in-use but old. Quote the replacement values the API surfaces — they tell the user what to inline when removing.
...
- **Don't infer "stale" yourself.** Use `/stale-features` for the canonical determination — it encodes GrowthBook's own staleness rules and surfaces replacement values you can't compute locally.
- **Rate limit is 60 rpm.** If listing a huge org with `/features` pagination, pace yourself.

## Endpoints used

- `GET /api/v1/feature-keys` — full ID list, no pagination cap. Cheapest call.
- `GET /api/v1/features` — paginated list with full configuration. `limit` up to 100, `offset` for paging.
- `GET /api/v1/features/{id}` — full configuration for one flag.
- `GET /api/v1/stale-features?ids=a,b,c` — staleness audit. Requires explicit `ids`.
