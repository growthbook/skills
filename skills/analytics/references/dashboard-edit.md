...
- **A failed comparison never fails a write.** Only the primary query does. A chart whose compare-to-previous-period run fails is saved without the comparison (or, on a carried tile, keeps the previous one), so a tile can come back showing a single series where the user asked for two.
