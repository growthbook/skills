...
- **Never guess column values for filters.** Use the `topValues` array on the fact table's string columns (`GET /api/v1/fact-tables/:id`) — it's refreshed by a background job, so it can be stale or absent. If the value you need isn't there, try the **ask-data** skill's preview-values endpoint, ask the user for the exact value, or fall back to a `contains` filter, and say which you did.
...
- the **ask-data** skill — when no metric or exploration can answer the question (custom joins, unmodeled tables, complex aggregations that the exploration config can't express)
