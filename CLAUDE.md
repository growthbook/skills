...
- A `200` from a `dryRun: true` bulk import is not success — a dry run collects non-permission validation failures instead of stopping at the first, so branch on `success` and read `errors[]` (`resourceType`, `id`, `message`). A permission failure short-circuits with `403` (metric-migrate)
...
- For `proportion`/`retention` metrics the fact-metric handler **silently overwrites** `numerator.column` with `$$distinctUsers` (and `dailyParticipation` with `$$distinctDates`) and clears `aggregation` — it does not error, so a wrong column yields a silently different metric rather than a failure. Funnel metrics have no numerator (metric-migrate)
...
- `GET /api/v1/metric-groups` uses the default `ApiModel` list validator and is unpaginated; its query schema is `z.never()`, so adding `?limit=100` or any other query parameter is rejected (metric-migrate)
...
- Analytics: `metric-search → metric-migrate → analytics-explore`
