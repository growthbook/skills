...
Four domains: `feature-flags` (17 workflows), `experiments` (6), `analytics` (4), and `gb-setup` (no `references/` — it's a single workflow).
...
- The public product-analytics surface is exactly the four `/api/v1/product-analytics/*-exploration` POSTs (`metric`, `fact-table`, `data-source`, `funnel`) — there is no search, columns, or column-values endpoint. A saved funnel fact metric is charted through `funnel-exploration`, not the ordinary `metric-exploration` path. Discovery goes through `/fact-metrics`, `/fact-tables`, and the information-schema endpoints, and a fact table's `columns[].topValues` is the only way to look up a column's values (analytics-explore, metric-search)
...
- `GET /api/v1/metrics` includes archived metrics by default (`includeArchived` defaults to `true`) — pass `includeArchived=false` to list only live ones (metric-migrate)
- `POST /api/v1/bulk-import/facts` defaults `managedBy` to `"api"` on any fact table or fact metric that omits it, which **disables UI editing**. Send `defaultManagedBy: ""` at the top level; a per-resource `managedBy` overrides it (metric-migrate)
- A `200` from a `dryRun: true` bulk import is not success — a dry run collects every failing resource instead of stopping at the first, so branch on `success` and read `errors[]` (`resourceType`, `id`, `message`) (metric-migrate)
- Bulk import is **not transactional**: a live run stops at the first failure and returns `400` (`403` for permissions) with the write counts and `errors[]`, leaving everything before it created. Re-fetch and re-run with deterministic ids so the retry upserts (metric-migrate)
- Funnel fact metrics require `funnelSettings` and **reject** `numerator` and `denominator` — sending either is a validation error, not a silent override. They are also gated on the `funnel-metrics` premium feature, which a dry run reports per metric (metric-migrate)
- For `proportion`/`retention`/`funnel` metrics the fact-metric handler **silently overwrites** `numerator.column` with `$$distinctUsers` (and `dailyParticipation` with `$$distinctDates`) and clears `aggregation` — it does not error, so a wrong column yields a silently different metric rather than a failure (metric-migrate)
- Ratio metrics support only `percentile` capping, but that rule lives **only in the UI** (`FactMetricModal.tsx`). The API accepts `absolute` capping on a ratio metric without complaint — skills must normalize it themselves (metric-migrate)
- Legacy metrics with `managedBy: "config"` cannot be archived via the API: `FILE_CONFIG_UPDATEABLE_FIELDS` allows only `analysis`/`analysisError`/`queries`/`runStarted`. They can still be migrated — the replacement's `replaces` surfaces the pointer on the legacy metric's page — but the original has to be retired in `config.yml` (metric-migrate)
- A fact metric's `replaces` array is the mechanism that keeps historical results readable across a migration. Experiment results render the **live** metric list against a frozen snapshot, so a metric swapped in after a snapshot ran would otherwise show no data; `resolveMetricsForSnapshot` substitutes the replaced metric's rows and flags them instead. It also drives the "replaced by" callout on the old metric's page. API-only, cannot contain the metric's own id, and replaced ids are not checked for existence — a typo fails silently (metric-migrate)
- `GET /api/v1/usage/metrics?ids=` is the batched metric-usage check and the right one: it expands metric groups server-side, which a reverse index over `GET /api/v1/experiments` alone cannot do (experiments store unexpanded group ids). It has no `status` filter (filter `experimentStatus` client-side) and only covers experiments the token can read — an under-report, not an error (metric-migrate)
- Metric groups **do** have a public CRUD API at `/api/v1/metric-groups`, registered via an `ApiModel` spec (`api/specs/metric-group.spec.ts`, `includeDefaultCrud: true`) rather than a handler directory — so `ls api/` does not reveal it. Check `api/specs/` before concluding a resource is unexposed (metric-migrate)
- Resources registered through an `ApiModel` spec with `includeDefaultCrud: true` get a list endpoint whose `querySchema` is `z.never()` (`ApiModel.ts`) — so `GET /api/v1/metric-groups?limit=100` is **rejected**, not filtered, and the listing is unpaginated. A spec can opt out via `crudValidatorOverrides.list`, which `/experiment-templates` does (it accepts `projectId`) and `/metric-groups` does not (metric-migrate)
- A legacy metric's six decision settings (`minPercentChange`, `maxPercentChange`, `minSampleSize`, `targetMDE`, `riskThresholdSuccess`, `riskThresholdDanger`) sit under `behavior` but are **top-level** fields on a fact metric. Omitting them on create silently substitutes org defaults rather than erroring, and `GET /api/v1/metrics` always returns them populated, so an inherited value is indistinguishable from an explicit one (metric-migrate)
- Experiment templates are **by-value**: `POST /api/v1/experiments` spreads `templateToPostExperimentDefaults(template)` into the payload at creation and persists the result, keeping `templateId` as provenance only. Editing a template is forward-only and never touches an existing experiment — the opposite of a metric group, which resolves at read time. Templates hold metric ids in `goalMetrics`/`secondaryMetrics`/`guardrailMetrics` (arrays) and `activationMetric` (a string), and may also hold `mg_...` group ids (metric-migrate)
- `POST /api/v1/experiment-templates/bulk-import` takes **full** create bodies in `data`, not partials (`templateMetadata`/`type`/`datasource`/`exposureQueryId`/`statsEngine`/`targeting` all required), so a partial round-trip silently rewrites the template. `PUT /api/v1/experiment-templates/:id` is the true partial. Both live under an `ApiModel` spec, as does `/metric-groups` (metric-migrate)
