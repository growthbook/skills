...
- `dashboard-create` and `dashboard-edit` — build and change general Analytics dashboards through `/api/v1/dashboards`. A chart block carries only its exploration `config`; the write runs each one server-side and wires the result onto the tile, so a whole dashboard is one call and one confirmation rather than an exploration POST per chart. Covers the block reference (chart, experimentation, markdown, SQL), 24-column layout arithmetic, the dashboard-wide filter bar and period comparison, and the read-then-write discipline an update needs — a `PUT` replaces the block list outright, so a partial list deletes the tiles it omits. Experiment-attached dashboards are out of scope and halt.
  - Documents the write's partial-failure semantics, which the response does not report: only a chart sent in full fails the call, while a tile carried by id and re-run because a dashboard-wide setting changed is best-effort and keeps its previous result on failure, and a failed compare-to-previous-period run never fails the write. Also notes that listing the same block id twice is rejected, alongside the existing note on an id the dashboard does not have.
